...
const filteredEntries = computed(() => {
    return entries.value.filter(entry => {
        ...
        if (filters.value.category && entry.category !== filters.value.category) return false
...
watch(
    () => [filters.value.type, filters.value.category, filters.value.dateRange, filters.value.description],
    () => {
        if (props.incomeExpenses) {
...
const addEntry = async () => {
    if (modalMode.value === "add") {
        ...
        router.put(route('income-expenses.update', currentId.value), { // Use router.put para atualizar
            amount: parseFloat(newEntry.value.amount.toString()),
            type: newEntry.value.type,
            description: newEntry.value.description,
            category: newEntry.value.category,
...
const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `income-expenses-${new Date().toISOString().split('T')[0]}.xlsx`)
}
    ...
    <MainLayout>
        <div class="space-y-6">
                ...
                <div>
                    <button @click="exportToExcel"
                        ...
                        <svg class="-ml-1 mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <rect x="3" y="8" width="13" height="13" rx="2" ry="2" stroke-width="2"
                                stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M15 3h6m0 0v6m0-6L10 14" stroke-width="2" stroke-linecap="round"
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="flex items-baseline">
                                        <div class="text-2xl font-semibold text-gray-900">${{ totalIncome }}</div>
                                    </dd>
                                </dl>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="flex items-baseline">
                                        <div class="text-2xl font-semibold text-gray-900">${{ totalExpenses }}</div>
                                    </dd>
                                </dl>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="flex items-baseline">
                                        <div class="text-2xl font-semibold text-gray-900">${{ balance }}</div>
                                    </dd>
                                </dl>
                            </div>
                        </div>
            ...
            <div class="bg-white shadow rounded-lg p-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                    ...
                    <div>
                        ...
                        <select id="date" v-model="filters.dateRange"
                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    ...
                    <div>
                        <label for="date" class="block text-sm font-medium text-gray-700">Description</label>
                        <input type="text" id="description" v-model="filters.description"
                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="entry in props.incomeExpenses.data" :key="entry.id">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDate(entry.date) }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button class="text-indigo-600 hover:text-indigo-900 mr-3"
                                    @click="showModal('edit', entry)">Edit</button>

                                <button class="text-red-600 hover:text-red-900"
                                    @click="showModal('delete', entry)">Delete</button>
                            </td>
                        </tr>
                        <tr v-if="!props.incomeExpenses.data || props.incomeExpenses.data.length === 0">
                ...
                <!-- Pagination -->
                <div v-if="props.incomeExpenses?.last_page > 1"
                    class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                    <div class="flex items-center justify-between">
                        <div class="flex-1 flex justify-between sm:hidden">
                            <button @click="handlePageChange(props.incomeExpenses?.current_page - 1)"
                                :disabled="props.incomeExpenses?.current_page === 1" :class="[
                            ...
                            <button @click="handlePageChange(props.incomeExpenses?.current_page + 1)"
                                :disabled="props.incomeExpenses?.current_page === props.incomeExpenses?.last_page"
                                :class="[
                                    'relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md',
                                    props.incomeExpenses?.current_page === props.incomeExpenses?.last_page
                                        ? 'bg-gray-100 text-gray-400'
                                        : 'bg-white text-gray-700 hover:bg-gray-50'
                                ]">
                                Next
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    Showing
                                    <span class="font-medium">{{ (props.incomeExpenses?.current_page - 1) *
                                        props.incomeExpenses?.per_page + 1 }}</span>
                                    to
                                    <span class="font-medium">{{ Math.min(props.incomeExpenses?.current_page *
                                        props.incomeExpenses?.per_page, props.incomeExpenses?.total)
                                        }}</span>
                                    of
                                    <span class="font-medium">{{ props.incomeExpenses?.total }}</span>
                                    results
                                </p>
                            </div>
                            <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                    ...
                                    <button v-for="page in props.incomeExpenses?.last_page" :key="page"
                ...
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div
                        ...
                        <div v-if="modalMode == 'add' || modalMode == 'edit'"
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label for="entryType" class="block text-sm font-medium text-gray-700">Type</label>
                                    <select id="entryType" v-model="newEntry.type"
                                        class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                        <option value="income">Income</option>
                                        <option value="expense">Expense</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="text" id="amount" v-mask="'money'" v-model="newEntry.amount"
                                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                </div>
                                <div>
                                    <label for="description"
                                        class="block text-sm font-medium text-gray-700">Description</label>
                                    <input type="text" id="description" v-model="newEntry.description"
                                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                </div>
                                <div>
                                    <label for="category"
                                        class="block text-sm font-medium text-gray-700">Category</label>
                                    <select id="category" v-model="newEntry.category"
                                        class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                        <option v-for="category in categories" :key="category" :value="category">
                                            {{ category }}
                                        </option>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-2 mt-1">
                                        <input type="date" id="date" v-model="newEntry.date" class="flex-grow focus:ring-indigo-500 focus:border-indigo-500 block shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                        <div>
                                            <input type="file" ref="fileInput" class="hidden" @change="handleFileImport" />
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button type="button"
                                ...
                                :class="modalMode == 'edit' || modalMode == 'add' ? 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500' : 'bg-red-600 hover:bg-red-700 focus:ring-red-500'"
                                @click="addEntry">
                                {{ modalMode == 'edit' || modalMode == 'add' ? 'Send Entry' : 'Delete' }}
                            </button>
                            <button type="button"
